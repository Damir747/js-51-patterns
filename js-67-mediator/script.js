...
const renderSection = (data, nameSection) => {
	const getRecipesBlock = () => {
		...
		data.forEach(function (element) {
			booksBlock += new Block(new Mediator()).render(element); //renderRecipes(element);
		});
		booksBlock += `</ul>`;
		return booksBlock;
	};
...
class Block {
	...
	render(data) {
		return `
    <li class='recipes__item'>
      <div class='recipes__inner' style='${this.createBorder()}'><img class='recipes__img' src='/static/images/content/${data.img}' alt='${data.title}'>
      <div class='recipes__box'>
        <p class='recipes__author'>
          Автор: ${data.author}
        </p>

...
const renderSection = (data, nameSection) => {
	const getRecipesBlock = () => {
		...
		data.forEach(function (element) {
			console.log(element);
			if (element.author !== 'Иван Кириллов' && element.title !== 'Картофель Айдахо') {
				booksBlock += new Recept(renderRecipes).render(element);
			}
			else {
				booksBlock += new Recept(renderRecipesNew).render(element);
			}
		});
		booksBlock += `</ul>`;
		return booksBlock;
...
const Recept = function (strategy) {
	this.strategy = strategy;
}

Recept.prototype.render = function (data) {

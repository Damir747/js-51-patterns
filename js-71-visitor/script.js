...
class Recipes {
	accept(operation, data) {
		return operation.visit(this, data);
	}
	render(data) {
		return `
...
const renderSection = (data, nameSection) => {
	const getRecipesBlock = () => {
		...
		const recipesVisitor = new RecipesVisitor();
		const titleVisitor = new TitleVisitor();
		data.forEach(function (element) {
			const recipes = new Recipes();
			recipes.accept(titleVisitor, element);
			booksBlock += recipes.accept(recipesVisitor, element);
		});
		booksBlock += `</ul>`;
		return booksBlock;
	};
...
class Visitor {
	visitRecipes(recipes) {
		throw new Error(`В ${this.constructor.name} не описан метод visitRecipes()`);
	}
}

class RecipesVisitor extends Visitor {
	visit(recipes, data) {
		return recipes.render(data);
	}
}

class TitleVisitor extends Visitor {
	visit(book, data) {
		if (data.title === 'Яблочный пирог') {
			data.discr = 'рецепт на данный момент в разработке';
		}
	}
}
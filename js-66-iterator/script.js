...
const renderBook = (data) => {
  ...
  <li class="books__item">
   <div class="books__inner">
      <img class="books__img" src="static/images/content/books/${data.img}" alt="${data.title}" width="275" height="320">
      <div class="books__box">
         <p class="books__author">${data.author}</p>
         <h3 class="books__name">${data.title}</h3>
         ...
         <div class="books__wrapper"><a class="books__link" href="#">Подробнее</a><span class="books__pages">${data.count_page} страниц</span>
         </div>
      </div>
   </div>
  </li>
...
class IteratorClass {
	constructor(data, done) {
		...
		for (let i = 0; i < data.length; i++) {
			if (!this.data[data[i].type]) {
				this.data[data[i].type] = [];
			}
			this.data[data[i].type].push(data[i]);
		}
	}
	next() {
		if (!this.done) {
			this.step++;
			const type = this.types[this.step];
...
const condition = (hours >= 10 && hours < 12);

getDataFromServer(`${window.location.origin}/data.json`, function (err, data) {
	const iterator = new IteratorClass(data, condition);
	for (let field in SECTION) {
		const sectionBooks = iterator.next(); 	//data.filter((book) => book.type === field);

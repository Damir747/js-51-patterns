...
const renderSection = (data, nameSection) => {
	const getRecipesBlock = () => {
		...
		data.forEach(function (element) {
			if (nameSection === 'new') {
				booksBlock += new NewRecipes().templateRecipes(element);
			}
			else if (nameSection === 'best') {
				booksBlock += new PopularRecipes().templateRecipes(element);
			}
			else {
				console.error(`Нет такого nameSection: ${nameSection}`);
			}
		});
		booksBlock += `</ul>`;
		return booksBlock;
...
class Recipes {
    ...
    <li class='recipes__item'>
      ...
      <div class='recipes__box'>
        ...
        <h3 class='recipes__name'>${this.recipes_title ? this.recipes_title : data.title}</h3>
  ...
  `;
	...
	addToTitle(data) {
		this.recipes_title = `<img class='recipes__icon' src='${this.pathToImg}'>${data.title}`;
	}
	templateRecipes(data) {
		if (data.type === 'new' || data.type === 'best') {
...
class NewRecipes extends Recipes {
	recipesTitle = '🆕';
	pathToImg = '/static/images/common/new.png';
	isNew() {
		return true;
	}
}

class PopularRecipes extends Recipes {
	recipesTitle = '⭐';
	pathToImg = '/static/images/common/popular.png';
	isBest() {
		return true;
	}
}
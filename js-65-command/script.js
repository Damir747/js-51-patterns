...
class Command {
	...
	Execute() {
		throw new Error(`Метод Execute() в классе ${this.constructor.name} не описан`);
	}
}
...
class JSONWorker extends Command {
	...
	getDataFromServer() {
		...
		xhr.onload = function () {
			...
			if (status === 200) {
				new JSONWorker().renderBook(null, JSON.parse(xhr.response));
			} else {
				new JSONWorker().renderBook(status, xhr.response);
			}
		};
		xhr.send();
	};
...
class GetDataCommand extends Command {
	...
	Execute() {
		this.receiver.getDataFromServer();
	}
}

let invoker = new Invoker();
let jsonWorker = new JSONWorker();
let getDataFromServer = new GetDataCommand(jsonWorker);

invoker.SetCommand(getDataFromServer);
invoker.Execute();
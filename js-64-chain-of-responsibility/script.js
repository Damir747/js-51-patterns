...
class Dish {
	...
	isAvailable(dishId) {
		let result = 0;
		for (let cafe of LOCAL_BASE) {
			if (cafe.CAFE === this.type) {
				for (let product of cafe.PRODUCTS) {
					if (product.ID === dishId) {
						result = product;
						result.title = cafe.CAFE;
					}
				}
			}
		}
	...
	check(dishId) {
		const dishObject = this.isAvailable(dishId);
		if (dishObject && dishObject.COUNT > 0) {
			this.title = `${dishObject.title}<br> приготовлен ${dishObject.COUNT} раз`;
		}
		else {
			if (this.nextObject) {
				this.nextObject.check(dishId);
			}
			else {
...
const checkAvalability = (dishId) => {
	const izabel = new Izabel();
	const rapunzel = new Rapunzel();
	rapunzel.setNext(izabel);
	return rapunzel.check(dishId);
}